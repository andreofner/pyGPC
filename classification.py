...
MNIST classification with static models and delayed modulation (active inference on weights)
"""
...
class Model(torch.nn.Module):
    ...
    def mask_grads(self, l, split=4, cause=False):
...
def feed_target(PCN):
    ...
    if not (TEST_TARGET and env_id > 0):  # evaluate target reconstruction
        PCN.curr_cause[-1] = torch.tensor(input.flip(-1).detach().float()).reshape(
            [B_SIZE, 1, -1])  # mirrored input image as target
...
def feed_observation(PCN):
    ...
    if not (TEST_INPUT and env_id > 0):  # evaluate input reconstruction
        #PCN.curr_cause[0] = torch.tensor(input.detach().float()).reshape([B_SIZE, 1, -1])
        PCN.curr_cause[0] = torch.zeros_like(PCN.curr_cause[0])
        #for b in range(B_SIZE):
        #    PCN.curr_cause[0][b, :, :10] = torch.ones_like(PCN.curr_cause[-1][b, :, :10]) * (target[b])  # digit as input
        for b in range(B_SIZE):
            for c in range(0,IMAGE_SIZE,16):
                PCN.curr_cause[0][b, :, c+target[b]] = 10  # one hot digit as input
...
def GPC(m, l, infer_precision=False, optimize=True, var_prior=7, covar_prior=10000000, transition=False, learn=False):
    """ Layer-wise Generalized Predictive Coding optimizer """

    B_SIZE_PRECISION_SLOW = 1 # 1
    ...
    if optimize: # optimize hierarchical prediction
        ...
        m.mask_grads(l+1, split=1, cause=True)  # part of cause states used for outgoing prediction
        m.mask_grads(l+1, split=0)  # part of hidden states used for outgoing prediction
        opt.step()
...
""" Network settings"""
UPDATES, B_SIZE, IMAGE_SIZE = 1, 128, 10  # model updates, batch size, input size
CONVERGENCE_THRESHOLD = .01  # prediction error threshold to stop inference
...
""" Experiment settings"""
ACTIONS = [1 for i in range(2)]  # number of datapoints
TEST_TARGET = False  # test dataset: reconstruct target given input
TEST_INPUT = True  # test dataset: reconstruct input given target

if __name__ == '__main__':
    #train_loader, test_loader = torchvision_mnist(batch_size=B_SIZE, size=16, shuffle=True)
    ...
    PCN = Model(STATE_SIZES, act=torch.nn.Identity(),  # hierarchical activation
                ...
                lr_w_d=[0.000 for i in range(len(STATE_SIZES))],  # dynamical weights learning rate
    ...
    print("Cause:", [c.shape for c in PCN.curr_cause])
    print("Hidden:", [c.shape for c in PCN.curr_hidden])
    ...
    for env_id, env_name in enumerate(['Train', 'Test'][:2]):
        ...
        PCN.init_covar() # todo covar in inference should have B_SIZE, not 1 like weights precision
        lowest_error = np.asarray([100.])

        last_weights = [copy.deepcopy(layer) for layer in PCN.layers]

        for a_id, (data, target) in enumerate(data_loader):

            if True:
                # compute prior accuracy for delayed weights updates
                input = data.reshape([B_SIZE, 1, -1]) * 10
                feed_target(PCN)
                pred_g_prior = predict(PCN, l=len(PCN.layers)-1, keep_states=False)
                feed_observation(PCN)
                _, e_h_prior, _ = GPC(PCN, l=0, infer_precision=True, transition=False)  # (l_h > 0 )  # step hierarchical variables
                accuracy_prior, pred_classes_prior, correct_prior = batch_accuracy(pred_g_prior, target)

                if True:
                    print(lowest_error)
                    if e_h_prior.mean().detach().abs().numpy() <= lowest_error:
                        print("Improved weights: ", e_h_prior.mean().detach().abs().numpy())

                        # reset slow precision to fast precision if prior prediction improved
                        PCN.covar_slow = [v.clone().detach().requires_grad_() for v in PCN.covar]
                        PCN.covar_hidden_slow = [v.detach().requires_grad_() for v in PCN.covar_hidden]
                        lowest_error = e_h_prior.mean().detach().abs().numpy()

                        # keep improved weights
                        last_weights = [copy.deepcopy(layer) for layer in
                                        PCN.layers]  # print(accuracy_prior, "last acc: ", lowest_error)
                    else:
                        # reject last weights update
                        # print("rejected weights!", accuracy_prior, "last acc: ", lowest_error)
                        PCN.layers = [layer for layer in last_weights]
            ...
            if True:  # optionally predict through entire network
                feed_target(PCN)
            ...
            for update in range(UPDATES):
                ...
                for l_h in reversed(range(len(PCN.layers))):
                    ...
                    if l_h == 0:
                        ...
                        #if HIDDEN_SPLIT != 0:
                        #    precisions_hidden.append(np.diag(np.array(PCN.covar_hidden[0].mean(dim=0).detach())).mean())
                        #    precisions_hidden_slow.append(np.diag(np.array(PCN.covar_hidden_slow[0].mean(dim=0).detach())).mean())
